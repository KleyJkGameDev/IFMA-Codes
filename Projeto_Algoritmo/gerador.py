...
while(contador < 15):
    ...
    caminho_arquivo = os.path.join(pasta_destin, f"rand_numb_{lg[contador]}.csv") # incrementar o nome do arquivo à pasta especificada
    print(caminho_arquivo)

    with open(caminho_arquivo, "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        #writer.writerow(["numeros"])
        for numero in tqdm(algo[l[contador]], desc="Gravando números", unit="linha"):
